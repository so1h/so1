...
pointer_t entornoMSDOS ( void ) 
{
    ...
    segEntorno = *((word_t far *)MK_FP(segPSP(), 0x002c)) ;
    ptrEntorno = MK_FP(segEntorno, 0x0000) ;
    return(ptrEntorno) ;
}
...
pointer_t valorMSDOS ( char * str ) 
{
    ...
    do {
        ...
        if ((ptrEntorno[i++] == '=') && (str[j] == '\0'))
            break ;
        else 
		{
            while (ptrEntorno[i] != '\0') i++ ;
        }
    } while (ptrEntorno[++i] != '\0') ;
...
int argcMSDOS ( void ) 
{
    ...
    tamCmdLine = (word_t)*((word_t far *)MK_FP(segPSP(), 0x0080)) ;
    ptrCmdLine = MK_FP(segPSP(), 0x0081) ;
