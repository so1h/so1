...
static int instalarConRat ( byte_t numConsolas ) {
  ...
  *((word_t far *)pointer(_CS, (word_t)segDatos)) = _DS ;     /* guardo DS */
  ...
  esperarDesinstalacion(0x0000, 0x0000) ;            /* bloquea el proceso */
...
int main ( int argc, char * argv [ ] ) {
  int res ;
  if (argc > 3) formato() ;
  else if (argc == 1) return(instalarConRat(numConsolasPorDefecto)) ;
  else if (argc == 2) {
    if (iguales(argv[1], "-h") || iguales(argv[1], "-H")) help() ;
    else if (iguales(argv[1], "-i") || iguales(argv[1], "-I"))
      return(instalarConRat(numConsolasPorDefecto)) ;
  ...
  if ((argc == 2) ||
    ...
    if (simb == s_numero) {
      ...
      else return(instalarConRat(num)) ;
    }
  }
  formato() ;
  return(-1) ;
