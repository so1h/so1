...
/*                                 leer_s.c                                */
/* ----------------------------------------------------------------------- */
/*                     programa leer_s para el sistema SO1                 */
/* ----------------------------------------------------------------------- */
...
void formato ( void ) {
  escribirStr("\n\n") ;
  escribirStr(" formato: leer_s [ unidad sector numero fichero | -h ] ") ;
  escribirLn() ;
}
...
void help ( void ) {
  escribirLn() ;
  escribirLn() ;
  escribirStr(" formato 1: leer_s ") ;
                           escribirStr("muestra el formato del comando. \n\n") ;
  escribirStr(" formato 2: leer_s -h ") ;
                                        escribirStr("muestra este help. \n\n") ;
  escribirStr(" formato 3: leer_s unidad sector nun fichero             \n\n") ;
...
void procesarOperacion ( word_t unidad, word_t sector, word_t numero, int df ) {
  ...
  for ( i = 0 ; i < numero ; i++ ) {
    ...
    escribirDec(prc, 6) ;
    escribirCar('%') ;
    codigo = leerSectorBIOS(sectorActual, (byte_t)unidad, (pointer_t)&buf) ;
    if (codigo) error = 1 ;
    else if ((escritos = writeDOS(df, (char *)&buf, 512)) != 512)
      error = 2 ;
