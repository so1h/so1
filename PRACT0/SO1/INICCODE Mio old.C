...
int inic ( void ) {                /* lanza los principales drivers de SO1 */
  ...
  printStrBIOS(", RETARDO&") ;
         ...
         /* qemu se retarda de mas, por lo tratamos de una manera especial */
...
//printStrBIOS("\n inic: tras createProcess TIMER") ;
  ...
  for ( i = 1 ; i <= numConsolas/2 ; i++ ) {
    ...
    open(nombre, O_RDONLY) ; open(nombre, O_WRONLY) ; open(nombre, O_WRONLY) ;
  ...
  close(STDIN) ; close(STDOUT) ; close(STDERR) ;
  open("CON0", O_RDONLY) ; open("CON0", O_WRONLY) ; open("CON0", O_WRONLY) ;
