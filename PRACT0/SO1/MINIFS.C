...
#include <so1pub.h\msdos.h>                   /* getDiskDOS, readDOSFar */
...
static int far readSF ( int dfs, pointer_t dir, word_t nbytes ) {
  int n ;
  n = readDOSFar(descFichero[dfs].menor, (char far *)dir, nbytes) ;
  return(n) ;
}
...
static int far writeSF ( int dfs, pointer_t dir, word_t nbytes ) {
  int n ;
  n = writeDOSFar(descFichero[dfs].menor, (char far *)dir, nbytes) ;
  return(n) ;
}
